...
def sigagg(sigmas):
    ...
    temp2 = G2mul(g2gen,0)
    for item in sigmas:
        temp1 = G1add(sigmas[item][0],temp1)
        temp2 = G2add(sigmas[item][1],temp2)
    aggsig = [temp1, temp2]
    return aggsig
...
def testcost():
    ...
    sigmas = {}
    num_user = 100
    print("Size testing begin:")
    f_sk_size = open('sk.txt', 'w+')
    f_pk_size = open('pk.txt', 'w+')
    for i in range(num_user):
        (pks[i], sks[i]) = keygen()
        sigmas[i] = sign(sks[i], 1)
        f_sk_size.write(str(sks[i]))
        f_pk_size.write(str(pks[i]))
    f_sk_size.close()
    f_pk_size.close()
    ssk = get_FileSize('sk.txt')
    spk = get_FileSize('pk.txt')
    print("==secret key size", ssk/num_user)
    print("==public key size", spk/num_user)

    sig0 = sign(sks[0], 1)

    f_signature_size = open('signature.txt', 'w+')
    f_signature_size.write(str(sig0))
    f_signature_size.close()
    ssignature = get_FileSize('signature.txt')
    print("==signature size", ssignature)

    print("Time testing begin:")
    print("==testing key generation time")
    start = time.time()
    (pk, sk) = keygen()
    end = time.time()
    print("==key generation time cost", end - start)

    print("==testing public key and signature aggregation time")
    start = time.time()
    keyagg(pks)
    end = time.time()
    print("==public key aggregation time cost", end - start)
    ...
    print("== testing sign, verify and key-update")
    time_sign = 0
    time_verify = 0
    time_update = 0
    iter = 1000
    ...
    print("==signing time", time_sign/iter)
    print("==verify time", time_verify/iter)
    print("==key update time", time_update/iter)
